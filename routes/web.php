<?php

use Illuminate\Support\Facades\Route;

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| contains the "web" middleware group. Now create something great!
|
*/

Route::get('/', function () {
    return view('welcome');
});

Auth::routes();

// Route::get('/admin', 'MainController@index')->name('admin');
// Route::get('/user', 'MainController@index')->name('user');

Route::group(['middleware' => ['auth']], function() {

    Route::get('/user', 'MainController@userMain')->name('user');
    Route::get('/permission-denied', 'MainController@permissionDenied')->name('nopermission');

    Route::group(['middleware' => ['admin']], function() {
        Route::get('/admin', 'AdminController@index')->name('admin');
        Route::get('/admin/remove-admin/{userId}', 'AdminController@removeAdmin');
        Route::get('/admin/give-admin/{userId}', 'AdminController@giveAdmin');
    });
    
});

Route::resource('/home', 'HomeController');
Route::resource('/patient', 'PatientController');
//Route::get('/home', 'HomeController@index')->name('home');

Route::resource('/userprofile', 'PatientController');
